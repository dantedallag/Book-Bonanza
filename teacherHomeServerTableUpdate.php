...
if(isset($_POST['title'])) {
    $title = $_POST['title'];
    $author = $_POST['author'];
    mysqli_query($connection,"DELETE FROM books WHERE title = '" . $title . "' AND author = '" . $author . "'");
    ...
    if($studentName == "All Books") {
        ...
        $studentName = $_POST['studentName'];
        $result = mysqli_query($connection, "SELECT title,author,lexile,page_length,genre,trait1,trait2,recommended FROM books INNER JOIN chosen ON chosen.book_id = books.id INNER JOIN users ON ('". $studentName . "' = users.person_name AND users.id = chosen.user_id)"); 
    }
}
